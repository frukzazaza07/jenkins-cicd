pipeline {
    ...
    stages {
                    ...
                    try{
                        ...
                        dir('application') { 
                            ...
                            docker.withRegistry("${env.DOCKER_REGISTRY_URL}", 'DOCKER-LOGIN-REGISTRY') {
                                ...
                                docker.build("${env.REPO_NAME}:${params.AppVersion}", "--no-cache --file .docker/php.dockerfile .").push()
                            }
                        }
        ...
        stage('Start new app version') {
            steps {
                script {
                        echo "🚀 Starting SSH Connecting to ${env.DEST_IP}"
                        def status = sshagent(['SSH_INSUREOK_SERVER']) {
                            sh(
                                script: "ssh -o StrictHostKeyChecking=no -p ${env.SSH_PORT} ${env.SSH_CREDENTIAL_USR}@${env.DEST_IP} \"cd /home/application/insureok && sudo docker compose down && sudo docker volume rm insureok_app-build-public && sudo docker compose up -d\"",
                                returnStatus: true
                            )
                        }

                        if (status == 0) {
                            echo "✅ Start new app version success"
                        } else {
                            echo "❌ Start new app failed"
                            error("❌ Start new app failed")
                        }
                }
            }
        }
